...
def run_bot(worker_id: int):
    cmd = [
        sys.executable,
        "-m",
        "orchestrators.smartsheet_dispatcher_he",
        str(worker_id),
    ]
...
def run_once():
    ...
    for worker_id in range(1, TOTAL_WORKERS + 1):
        ...
        processes.append(p2)
        print(f"[{datetime.now()}] Worker {worker_id} classificacao_he iniciado. PID={p2.pid}")
...
def main():
    ...
    while True:
        if STOP_EVENT.is_set():
            print(f"[{datetime.now()}] Encerramento gracioso do worker classificacao_he concluido.")
            break
        ...
        try:
            ...
            print(f"[{datetime.now()}] Erro no ciclo HE_PV: {e}")

        if STOP_EVENT.is_set():
            print(f"[{datetime.now()}] Encerramento gracioso do worker classificacao_he concluido.")
            break

        print(f"[{datetime.now()}] Proximo ciclo HE_PV em {INTERVAL_SECONDS}s")
