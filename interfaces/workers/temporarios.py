...
def run_once():
    ...
    for worker_id in range(1, TOTAL_WORKERS + 1):
        ...
        processes.append(p2)
        print(f"[{datetime.now()}] Worker {worker_id} temporarios iniciado. PID={p2.pid}")
...
def main():
    ...
    while True:
        if STOP_EVENT.is_set():
            print(f"[{datetime.now()}] Encerramento gracioso do worker temporarios concluido.")
            break
        ...
        print(f"[{datetime.now()}] Proximo ciclo temporarios em {INTERVAL_SECONDS}s")
