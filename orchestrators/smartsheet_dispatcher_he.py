...
from services.classificacao_he.posto_vago_service import PostoVago
...
class SmartsheetDispatcher:
    ...
    def main(worker_id: int, total_workers: int = 1):
        ...
        def _handle_shutdown(signum, _frame):
            ...
            stop_requested = True
            print(f"[dispatcher_he] Sinal {signum} recebido. Encerrando com flush pendente...")

        signal.signal(signal.SIGTERM, _handle_shutdown)
        signal.signal(signal.SIGINT, _handle_shutdown)
        sheet, _, token, sheet_id, _ = SmartsheetClient.setup_smartsheet(settings.SHEET_ID_CLASSIFICACAO_HE)
        ...
        send_execution_mapping("classificacao_he", "Classificacao posto vago", 0, execution_start_time, running=True, starting=True)
        all_updates = []
        ...
        try:
            for linha in bloco_linhas:
                updates = []
                if stop_requested:
                    print("[dispatcher_he] Interrupcao solicitada, finalizando loop para aplicar update_bulk.")
                    break
                dados_celulas = {
                ...
                id_mrh            = dados_celulas.get('N° MRH', None)
                id_mrh            = str(int(float(id_mrh))) if id_mrh is not None else None
                cr_cobertura      = dados_celulas.get('CR Cobertura', None)
                
                cr_cobertura      = str(cr_cobertura[:5]) if cr_cobertura is not None else None
                classificacao_he  = dados_celulas.get('Classificação da H.E.', None)
                ...
                if status is None:
                    ...
                    match str(classificacao_he).strip().lower():
                        case "posto vago":
                            service = PostoVago(
                                driver=driver,
                                row_id=row_id,
                    ...
                    if updates:
                        ...
                        if len(all_updates) >= batch_size:
                            ...
                            try:
                                SmartsheetClient.update_bulk(all_updates, settings.SHEET_ID_CLASSIFICACAO_HE)
                            finally:
                                send_execution_mapping("classificacao_he", "Classificacao posto vago", batch_count, batch_start_time, running=True)
                                all_updates.clear()
                                batch_start_time = datetime.now()
            ...
            if all_updates:
                ...
                try:
                    ...
                    send_execution_mapping("classificacao_he", "Classificacao posto vago", final_count, batch_start_time, running=False)
            else:
                print("[dispatcher_he] Nenhum update pendente; enviando status para API com 0 linhas.")
                send_execution_mapping("classificacao_he", "Classificacao posto vago", 0, batch_start_time, running=False)
